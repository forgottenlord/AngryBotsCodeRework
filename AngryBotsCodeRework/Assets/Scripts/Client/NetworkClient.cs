...
namespace Client
{
    ...
    public class NetworkClient : MonoBehaviour
    {
        ...
        IPEndPoint endPoint;
        #endregion
        [SerializeField] List<Vector3> debugClientPositions = new List<Vector3>();
        ...
        void Awake()
        {
            ...
            // n означает нового пользователя
            // сервер ответит уникальным идентификатором этого пользователя
            TryConnectToServer();
        ...
        {
            int n = 0;
            foreach (var client in otherClients.Values)
            {
                if (debugClientPositions.Count < otherClients.Values.Count)
                    debugClientPositions.Add(new Vector3());
                debugClientPositions[n] = client.transform.position;
